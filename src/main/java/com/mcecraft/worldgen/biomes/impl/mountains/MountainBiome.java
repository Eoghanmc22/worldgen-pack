...
import com.mcecraft.worldgen.biomes.BaseBiome;
...
public abstract class MountainBiome extends BaseBiome {
	...
	public MountainBiome(WorldGen wg, Biome minestomBiome, Plate plate, short overlayBlock, int overlayElevation, short surfaceBlock, short topBlock, int topLayers, PlaceableFeature... features) {
	...
	public void generate(ChunkBatch batch, int x, int z, int height, int chunkX, int chunkZ, int biomeId, ChunkRandom rng) {
		...
		if (height < 63) {
			...
			} else if (overlayBlock != -1 && rng.nextInt(overlayElevation - height+1) == 0) {
				batch.setBlockStateId(x, height+1, z, overlayBlock);
			}
		}
