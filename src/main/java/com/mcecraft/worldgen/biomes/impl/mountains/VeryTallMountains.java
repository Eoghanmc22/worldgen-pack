...
public class VeryTallMountains extends MountainBiome {

	private static VeryTallMountains INSTANCE;

	private static final Biome BIOME = Biome.builder().name(NamespaceID.from("wgp:tall_mountains")).build();

	public VeryTallMountains(WorldGen wg) {
	...
	public static VeryTallMountains getInstance() {
		return INSTANCE;
	}

	@Override
	public int getHeight(int x, int z, int biomeId) {
		return (int) (185+Math.abs(getPlateNoise(x,z, 3)) + noise1.getNoise(x, z)*30);
	}
}
