...
public class MountainsInnerLayer extends MultiSamplingLayer {

    public MountainsInnerLayer(long baseSeed) {
        super(baseSeed);
    }
    ...
    protected int genBiomes(int x, int z, int u, int d, int r, int l, int c, ChunkRandom rng) {
        ...
        if (Layer.cmpBiomeClimate(u, climate, biome) || Layer.cmpBiomeClimate(d, climate, biome) || Layer.cmpBiomeClimate(l, climate, biome) || Layer.cmpBiomeClimate(r, climate, biome)) {
            ...
            return Layer.setIsLand(0, true) | Layer.setClimate(0, MountainsSteep.getInstance().getClimateId()) | Layer.setBiomeId(0, MountainsSteep.getInstance().getBiomeId());
        }
        return c;
    }
}
