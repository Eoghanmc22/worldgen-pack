...
public class DynamicMountainsLayer extends MultiSamplingLayer {

	private final int data;

	public DynamicMountainsLayer(long baseSeed, int height) {
		super(baseSeed);
		this.data = height;
	}
	...
	protected int genBiomes(int x, int z, int u, int d, int r, int l, int c, ChunkRandom rng) {
		...
		if (Layer.cmpBiomeClimate(u, climate, biome) && Layer.cmpBiomeClimate(d, climate, biome) &&
				Layer.cmpBiomeClimate(r, climate, biome) && Layer.cmpBiomeClimate(l, climate, biome)) {
			return c;
		}
